...
gaussDisplace_modified <- function(mesh1,mesh2,sigma,gamma=2,W0,f,oneway=F,nh=NULL,tol=0,pro=c("morpho","vcg","kd"),k0=50,prometh=1,rhotol=NULL,border=FALSE,horiz.disp=NULL,angclost=FALSE,bbox=NULL,threads=1,smoothtype=0,...) {
...
gaussMatch_modified <- function(x,mesh2,mesh2_lms=NULL,mesh1lms_indicesR=NULL,mesh1lms_indicesL=NULL,mesh1lms_indicesSagittal=NULL,iterations=10,smooth=NULL,smoothit=10,radius4spheres3d=.2,smoothtype=c("taubin","laplace","HClaplace"),sigma=20,displacementsmooth=c("Gauss","Laplace","Exponential"),gamma=2,f=1.2,oneway=F,lm1=NULL,lm2=NULL,rigid=NULL, similarity=NULL, affine=NULL,tps=FALSE,nh=NULL,toldist=0,pro=c("kd","vcg","morpho"),k0=50,prometh=1,angtol=pi/2,border=FALSE,horiz.disp=NULL,useiter=FALSE,AmbergK=NULL,AmbergLambda=NULL,tol=1e-5, useConstrained=TRUE, angclost=TRUE,noinc=FALSE,silent=FALSE, visualize=FALSE,folder=NULL,alpha=0.7,col1="red",col2="white",bbox=NULL,bboxCrop=NULL,threads=0,cb=NULL,...) {
  ...
  if (visualize) {
    ...
    if (!length(rgl.ids()$id)) 
      open3d(windowRect=c(80,80,800,800),zoom = .5)
    else {
      rgl.bringtotop()
      rgl.clear()
    }
